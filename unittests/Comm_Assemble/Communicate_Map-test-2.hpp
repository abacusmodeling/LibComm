...
#include <tuple>
#include <fstream>
#include "Comm_Assemble/Comm_Assemble.h"
#include "example/Communicate_Map-1.h"
#include "example/Communicate_Map-2.h"
...
namespace Communicate_Map_Test
{
	void test_assemble()
	{
		...
		else if(MPI_Wrapper::mpi_get_rank(MPI_COMM_WORLD)==3){ m_in[2][1]=7.8;	m_in[1][10]=9.0; }

		Communicate_Map::Judge_Map2<int,int> judge;
		...
		*/
		...
		Comm_Assemble<
			...
			Communicate_Map::Judge_Map2<int,int>,
			std::map<int,std::map<int,double>>
		> com(MPI_COMM_WORLD);

		com.traverse_keys_provide = Communicate_Map::traverse_keys<int,int,double>;
		com.get_value_provide = Communicate_Map::get_value<int,int,double>;
		com.set_value_require = Communicate_Map::set_value_add<int,int,double>;
